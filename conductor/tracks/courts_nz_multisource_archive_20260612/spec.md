...
3. Email ingress:
   - GitHub cannot directly receive email as a mailbox. Implement an inbound email bridge using one of:
     - Cloudflare Email Routing Worker that receives the subscription email and calls GitHub `repository_dispatch` or commits raw email payloads through the GitHub API.
     - Mailgun inbound parse route that posts normalized email JSON to a small receiver which calls GitHub `repository_dispatch`.
...
4. Dataset publication:
   ...
   - Include raw-source bundles and normalized JSONL/Parquet outputs.
