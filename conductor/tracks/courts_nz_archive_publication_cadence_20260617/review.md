...
|---|-------------|--------|----------|
| R1 | Archive capture independent of outbound syndication | âœ… **Pass** | `archive_sources.yml` runs every 6h (`7 */6 * * *`) and is completely separate from `syndicate.yml` (15-min cron) and `publish_archives.yml` (weekly). Guardrail `archive_capture_independent_of_outbound_syndication: true` in cadence config. |
| R2 | Regular Hugging Face dataset updates | âœ… **Pass** | `publish_archives.yml` scheduled weekly (`41 16 * * 0`). Scheduled runs use `ARCHIVE_PUBLICATION_TARGET=huggingface`. Requires `HF_TOKEN` secret. |
| R3 | Zenodo as deliberate citable snapshot lane | âœ… **Pass** | `publish_zenodo_deposition.yml` is manual-only (`workflow_dispatch`), requires explicit `confirm: publish-zenodo-doi` string. Cadence: `manual_release_snapshot`. Guardrail: `scheduled_runs_must_not_publish_zenodo: true`. |
| R4 | Manual publish defaults to artifact-only | âœ… **Pass** | `publish_archives.yml` default `publish: false`. `ARCHIVE_PUBLICATION_TARGET` resolves to `artifact` when not scheduled and publish is false. Script only calls `--publish` when target is not `artifact`. |
| R5 | Publication status visible from committed reports | âœ… **Pass** | `conductor/archive_publication_status.json` written by `publish_archives.py`, committed by `commit_state_updates.py`. Records `mode`, `requested_targets`, `artifact`, `hugging_face`, `zenodo`. |
...
| Phase | Tasks | Status |
|-------|-------|--------|
| **Phase 1:** Current Behavior Audit | 3/3 | âœ… Complete |
| **Phase 2:** Hugging Face Cadence | 3/3 | âœ… Complete |
| **Phase 3:** Zenodo Cadence | 3/3 | âœ… Complete |
| **Phase 4:** Closeout | 3/3 | âœ… Complete |
...
|----------|-------------|------------|
| `publish_archives.yml` | Scheduled vs manual target routing via `ARCHIVE_PUBLICATION_TARGET` env var | âœ… Clean expression: scheduled â†’ `huggingface`, manual with `publish=true` â†’ user choice, manual without publish â†’ `artifact`. |
| `publish_zenodo_deposition.yml` | `publish-zenodo-doi` confirmation gate | âœ… Deliberate friction: requires exact string match, no accidental DOI publication. |
| `archive_sources.yml` | Independent 6-hourly capture | âœ… Completely decoupled from syndication and publication workflows. |

### Test coverage
| Test | Purpose | Status |
|------|---------|--------|
| `test_archive_publication_cadence_config.py` | Validates cadence config guardrails | âœ… 1 passed |
...
### âš ï¸ Minor Issue
1. **`archive_sources.yml` still uses `pip` instead of `uv`** â€” Line 23 uses `python -m pip install -r requirements.txt`. The `publish_archives.yml` (line 60) also still uses `pip`. These predate this track and are out of scope for the cadence change, but are inconsistent with the `workflow.md` recommendation and the migration done in `ci.yml` and `syndicate.yml`.
...
| Cadence contract machine-checkable | âœ… **Pass** |
| Guardrails documented and enforced | âœ… **Pass** |
| Workflows implement cadence correctly | âœ… **Pass** |
| Publication status committed and visible | âœ… **Pass** |
