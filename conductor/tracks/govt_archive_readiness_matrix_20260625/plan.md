...
- [ ] Task 10: Run `$conductor-review` after the matrix schema and first generated report exist.
- [ ] Task 11: Apply review fixes before marking the track complete.
- [ ] Task 12: Record commit notes that identify the next dependent track to run.
...
## Phase 1: Inventory Contract
- [ ] Task 1: Define one machine-readable readiness row per agency/source/profile with stable IDs.
- [ ] Task 2: Represent status as discovered, registered, resolver_ok, capture_ok, normalized_ok, published_ok, blocked_credential, blocked_legal, blocked_technical, stale, or retired.
- [ ] Task 3: Add source-type required fields for website_page, rss, bluesky, youtube, newsletter, facebook, instagram, threads, linkedin, and x.

## Phase 2: Dependency Model
- [ ] Task 4: Encode dependency sequencing so adapter work cannot claim success before registry and resolver gates pass.
- [ ] Task 5: Separate archive-only sources from outbound-mirror sources.
- [ ] Task 6: Preserve credential-gated platforms as explicit pending/onboarding records rather than failed captures.

## Phase 3: Reports
- [ ] Task 7: Generate `conductor/govt_archive_readiness_matrix.json`.
- [ ] Task 8: Generate a markdown summary grouped by agency and source type.
- [ ] Task 9: Expose counts for total discovered, registered, capturable without credentials, captured, published, and blocked.
